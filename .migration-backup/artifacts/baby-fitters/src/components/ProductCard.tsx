    ...
    >
      <Link to={`/product/${product.id}`} className="group block relative">
        <div className="aspect-[4/5] bg-cream overflow-hidden relative rounded-2xl">
          ...
          {(product.is_new_arrival || product.is_best_seller || hasDiscount) && (
            <div className="absolute top-4 left-4 flex flex-col gap-1.5">
              {hasDiscount && (
                <span className="px-2.5 py-1 rounded-full bg-background/95 backdrop-blur text-foreground text-[11px] font-bold uppercase tracking-[0.14em]">
              ...
              {product.is_best_seller && (
                <span className="px-2.5 py-1 rounded-full bg-foreground text-background text-[11px] font-bold uppercase tracking-[0.14em]">
                  Bestseller
                </span>
              )}
            </div>
          ...
          <button
            onClick={handleAdd}
            className="absolute bottom-3 left-3 right-3 h-11 rounded-full bg-background/95 backdrop-blur text-foreground text-[12px] font-bold uppercase tracking-[0.18em] flex items-center justify-center gap-2 opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-foreground hover:text-background shadow-soft"
        ...
        <div className="pt-4 px-1">
          <h3 className="text-[16px] font-semibold text-foreground line-clamp-2 tracking-tight leading-snug">
            {product.name}
          </h3>
          <div className="mt-2 flex items-baseline gap-2.5">
            <p className="text-[16px] font-bold text-foreground tracking-tight">{formatPrice(finalPrice)}</p>
            {hasDiscount && (
              <p className="text-[14px] text-muted-foreground line-through font-normal">{formatPrice(product.price)}</p>
            )}
          </div>
        </div>
      </Link>
